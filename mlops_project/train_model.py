...
for epoch in range(epochs):
    ...
    for batch in validation_dataloader:
        ...
        with torch.no_grad():
            # Forward pass
            eval_output = model(b_input_ids, token_type_ids=None, attention_mask=b_input_mask, labels=b_labels)
    ...
    print(
        "\t - Validation Precision: {:.4f}".format(sum(val_precision) / len(val_precision))
